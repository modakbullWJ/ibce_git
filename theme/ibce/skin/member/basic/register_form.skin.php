...
<div class="mbskin">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tbody>
        <tr>
            <th scope="row"><label for="reg_mb_id">★ 아이디 [账号]<strong class="sound_only">필수</strong></label></th>
            <td>
...
<!-- 이메일 Confirm -->
        ...
        <tr>
            <th scope="row"><label for="reg_mb_tel">전화번호 [电话号码]<?php if ($config['cf_req_tel']) { ?><strong class="sound_only">필수</strong><?php } ?></label></th>




            <td>
              <select name='tel_ccode' id="tel_ccode">
            ...
            <input type="text" name="mb_tel" value="<?php// echo get_text($member['mb_tel']) ?>" id="reg_mb_tel" <?php echo $config['cf_req_tel']?"required":""; ?> class="frm_input <?php echo $config['cf_req_tel']?"required":""; ?>" maxlength="20">

             <div>기존 전화번호<?php echo get_text($member['mb_tel'])?></div>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td>
              ...
              <select name='hp_ccode' id="hp_ccode">
                  <option value="">Country code</option>
                  <option value="+82">Korea +82</option>
                  <option value="+81">Japan +81</option>
                  <option value="+86">China +86</option>
                ...
                <input type="text" name="mb_hp" value="<?php //echo get_text($member['mb_hp']) ?>" id="reg_mb_hp" <?php echo ($config['cf_req_hp'])?"required":""; ?> class="frm_input <?php echo ($config['cf_req_hp'])?"required":""; ?>" maxlength="20">
                    ...
                    <div>기존 휴대폰번호<?php echo get_text($member['mb_hp'])?></div>
        ...
        <tr>
            ...
            <td>
              ...
              <input type="text" name="gr_tel" value="" maxlength="15" class ="frm_input" placeholder="">
             <div>기존 단체 전화번호<?php echo ($member['mb_3'])?></div>
              <!-- 2016.07.27 전우찬 -->
          </td>
      </tr>
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            <th scope="row"><label for="reg_mb_mailling">Mailing Service</label></th>
            <td>
                <input type="checkbox" name="mb_mailling" value="1" id="reg_mb_mailling" <?php echo ($w=='' || $member['mb_mailling'])?'checked':''; ?>>
                정보 메일을 받겠습니다. / Accept getting information on E-mail.
            </td>
        </tr>
...
var id_callback = function(){
    ...
    if(msg == "" || msg == null){
// 중복된 아이디가 존재하지 않는다.
if(!confirm("가입할 수 있는 아이디입니다.\n현재 아이디를 사용하시겠습니까?\n You can use this ID. Do you want to use this?")){
...
$("#reg_mb_id").keypress(function() {
    if (event.which == 13) id_callback();
});
$("#idcheck").click(id_callback);


// 이메일 중복체크
var pw_callback = function(){
    var msg = reg_mb_email_check();
    if(msg == "" || msg == null){
// 중복된 이메일이 존재하지 않는다.
if(!confirm("가입할 수 있는 이메일입니다.\n현재 이메일를 사용하시겠습니까?\n You can use this E-Mail. Do you want to use this?")){
...
$("#reg_mb_email").keypress(function() {
    if (event.which == 13) pw_callback();
});
$("#emailcheck").click(pw_callback);
    ...
    function fregisterform_submit(f)
    {
        ...
        {
            alert("입력하신 비밀번호와 비밀번호확인이 일치하지 않습니다. Password does not match.");
            return false;
        }

        if(f.mb_password.value.length < 8)
        {
            alert("비밀번호는 8 자리 이상으로 입력해주세요.\n\n You should type the password at least 8 letters. ");
            return false;
        }

        if(!f.mb_password.value.match(/([a-zA-Z0-9].*[!,@,#,$,%,^,&,*,?,_,~])|([!,@,#,$,%,^,&,*,?,_,~].*[a-zA-Z0-9])/))
        {
            alert("비밀번호는 문자, 숫자, 특수문자 3개를 모두 사용하셔야 합니다.\n\n You should type mix all of letter, number, special character.");
            return false;
        }
