...
<!-- 서브페이지 상단 백그라운드 불러오기 -->
<?php include_once(G5_THEME_MOBILE_PATH.'/inc/sub/m_sub_navi.php');?>
...
<table width="<?=$width?>" border=0 cellpadding="0" cellspacing="0">
  <tr>
   ...
   <td width="60%" align="center" style="margin-bottom:20px; height:50px;">
    <table border="0" cellspacing="5" cellpadding="0">
      <tr>
       <td><a href="<?php echo $_SERVER[PHP_SELF]."?bo_table=".$bo_table."&"; ?><?if ($month == 1) { $year_pre=$year-1; $month_pre=$month; } else {$year_pre=$year-1; $month_pre=$month;} echo ("year=$year_pre&month=$month_pre&sc_no=$sc_no");?>" title="전년도">≪<!--img src="<?=$board_skin_url?>/img/y_prev.gif" border="0" alt="<?=$year_pre?>년"--></a></td>
       <td><a href="<?php echo $_SERVER[PHP_SELF]."?bo_table=".$bo_table."&"; ?><?if ($month == 1) { $year_pre=$year-1; $month_pre=12; } else {$year_pre=$year; $month_pre=$month-1;} echo ("year=$year_pre&month=$month_pre&sc_no=$sc_no");?>" style="color:#ba9765; font-size:12px;" title="전월">◀<!--img src="<?=$board_skin_url?>/img/m_prev.gif" border="0" alt="<?=$month_pre?>월"--></a></td>
       <td style="padding:0 10px;font-size:16px;font-weight:bold;"><a href="<?php echo $_SERVER[PHP_SELF]."?bo_table=".$bo_table; ?>" title="오늘로" onfocus="this.blur()" style="font-size:16px; color:#ba9765;"><? echo ("$year".년."&nbsp;$month".월); ?></a></td>
       <td><a href="<?php echo $_SERVER[PHP_SELF]."?bo_table=".$bo_table."&"; ?><?if ($month == 12) { $year_pre=$year+1; $month_pre=1; } else {$year_pre=$year; $month_pre=$month+1;} echo ("year=$year_pre&month=$month_pre&sc_no=$sc_no");?>" style="color:#ba9765; font-size:12px;" title="후월">▶<!--img src="<?=$board_skin_url?>/img/m_next.gif" border="0" alt="<?=$month_pre?>월"--></a></td>
       <td><a href="<?php echo $_SERVER[PHP_SELF]."?bo_table=".$bo_table."&"; ?><?if ($month == 12) { $year_pre=$year+1; $month_pre=$month; } else {$year_pre=$year+1; $month_pre=$month;} echo ("year=$year_pre&month=$month_pre&sc_no=$sc_no");?>" title="후년도">≫<!--img src="<?=$board_skin_url?>/img/y_next.gif" border="0" alt="<?=$year_pre?>년"--></a></td>
     </tr>
   </table>
 </td>
 <td width="20%" align="right">
  <?php if ($rss_href || $write_href) { ?>
    <ul class="btn_bo_user">
      ...
      <?php if ($admin_href) { ?><li><a href="<?php echo $admin_href ?>" class="btn_admin"><font color="#ffffff">관리자</font></a></li>
      <li><a href="<?php echo $write_href ?>" class="btn_b02"><font color="#ffffff">일정추가</font></a></li><?php } ?>
    </ul>
    <?php } ?>
  </td>
</tr>
...
$lastcount = $lastday[$month]+$dayoftheweek + $temp;
  ...
  for ($iz = 1; $iz <= $lastcount; $iz++) { // 42번을 칠하게 된다.
    ...
    if ($b_year==$year && $b_mon==$month && $b_day==$cday) $bgcolor = "#fafafa" ;      //  "#DFFDDF"; // 오늘날짜 연두색으로 표기
	...
	$daycontcolor = "" ;
	$daycolor = "";
	if ($iz%7 == 1) $daycolor = "#ff6d6d"; // 일요일
	if ($iz%7 == 0) $daycolor = "#78a4fd"; // 토요일
  ...
  else { echo ("     <td width=$col_width height=$col_height bgcolor=fcf9f4 valign=top>&nbsp;</td>"); }
  if (($iz%7) == 0) echo ("  </tr>");
