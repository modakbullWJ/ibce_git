...
<div class="local_desc01 local_desc">
    ...
    <p>
        <strong># 회원정보 수정 시 참고사항 #</strong>
    </p>

    <p>
        <strong>회원등급(시험급수)</strong>의 수정은 좌측의 체크박스를 선택 후 선택수정 버튼을 누르셔야 합니다.
    </p>
    <p>
        또한, <strong>합격여부 및 불합격횟수 관리</strong>의 경우 변경시 바로 반영되니 이용에 참고해주시길 바랍니다.
    </p>
</div>
...
<div class="tbl_head02 tbl_wrap">
    ...
    <thead>
        <tr>
            ...
            <!-- 회원고유코드 추가 wj-->
            <th scope="col" id="mb_list_join" rowspan="2"><?php echo subject_sort_link('mb_1', '', 'desc') ?>회원고유코드</a></th>
            <!-- 원래 있떤거 -->
            ...
            <!-- 합격관련 tr추가  wj -->
            <th scope="col" rowspan="2" id="mb_list_join"><?php echo subject_sort_link('mb_8', '', 'desc') ?>합격여부</a></th>
            <th scope="col" rowspan="2" id="mb_list_join"><?php echo subject_sort_link('mb_10', '', 'desc') ?>불합격횟수 관리</a></th>
        </tr>
    ...
    <tr class="<?php echo $bg; ?>">
        <td headers="mb_list_chk" class="td_chk" rowspan="2">
            ...
            <input type="checkbox" name="chk[]" value="<?php echo $i ?>" id="chk_<?php echo $i ?>">
                ...
                <!-- 회원고유코드 추가 테스트-->
        <td headers="mb_list_join" class="td_date" rowspan="2"><?php echo ($row['mb_1']); ?></td>
       ...
       <!-- <td headers="mb_list_grp" rowspan="2" class="td_numsmall"> --><?php //echo $group ?><!-- </td> -->
        ...
        ?>
        <!-- 합격 불합격 표시 wj -->
        <td headers="mb_list_join" rowspan="2" class="td_date">


           <select name="mb_8" id="mb_8_<?php echo $i;?>" onchange="mb8_chk( '<?php echo $i;?>', '<?php echo $row['mb_id']?>',this.value);" >

            <option value=""></option>
            <option value="PASS" <? if($row['mb_8'] == 'PASS') echo " selected "; ?>>PASS</option>
            <option value="FAIL" <? if($row['mb_8'] == 'FAIL') echo " selected "; ?>>FAIL</option>
        ...
        <!-- 불합격 횟수 관리 wj 스크립트의 ajax와 연동 -->
        <td headers="mb_list_join" rowspan="2" class="td_date">

           <select name="mb_10" id="mb_10_<?php echo $i;?>" onchange="mb10_chk( '<?php echo $i;?>', '<?php echo $row['mb_id']?>',this.value);" >

            <option value="" <? if($row['mb_10'] == '') echo " selected "; ?>>0</option>
            <option value="2" <? if($row['mb_10'] == '2') echo " selected "; ?>>1</option>
            <option value="3" <? if($row['mb_10'] == '3') echo " selected "; ?>>2</option>
            <option value="4" <? if($row['mb_10'] == '4') echo " selected "; ?>>3(응시불가)</option>
...
// 셀렉트 박스를 바꿔서 클릭할때마다 감지해서 ajax로 값 보내기
var mb10_chk = function(chk_num, mb_id,field) {

$.ajax({
    type:"post",
    url:"<?php echo $G5_URL?>/adm/ajax_member_fcount_update.php",
    data:
    {
        chk_num:chk_num,
        mb_id : mb_id,
    ...
    success:function(res){
        ...
        $("#mb_10_"+chk_num).val(res);

        if(res){

            alert("불합격횟수가 " + res + "(으)로 변경되었습니다");
        ...
        location.reload();
        // db변경된 값이 셀렉트박스에 바로 표시가 되야 되는데... 안되서 새로고침하게 만듦 wj
    },
...
// 합격 불합격
var mb8_chk = function(chk_num, mb_id,field) {

$.ajax({
    type:"post",
    url:"<?php echo $G5_URL?>/adm/ajax_member_pf_update.php",
    data:
    {
        chk_num:chk_num,
        mb_id : mb_id,
